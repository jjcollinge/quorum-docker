...
docker build -t $SERVICE_IMAGE_ID -f Dockerfile .
. ./env.sh
ARGS="--name $SERVICE_CONTAINER_ID --rm ${OPTIONAL_ARGS} -e BOOTNODE_IP=${BOOTNODE_IP} -e BOOTNODE_PORT=${BOOTNODE_PORT} -e RPC_PORT=${RPC_PORT} -e CONSTELLATION_PORT=${CONSTELLATION_PORT} -e GETH_PORT=${GETH_PORT} --net=host ${SERVICE_IMAGE_ID}"
