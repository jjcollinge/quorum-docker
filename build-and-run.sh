...
if [ ! -d "target" ]; then
        ...
        echo "Building image ${BUILD_IMAGE_ID}"
        docker build -t $BUILD_IMAGE_ID -f Dockerfile.build .
        echo "Creating container ${BUILD_INSTANCE_ID}"
        ...
        echo "Artefacts in target folder already exist, skipping build"
fi

SERVICE_IMAGE_ID="service${UNIQUE_ID}"
SERVICE_CONTAINER_ID="${SERVICE_IMAGE_ID}-container"
echo "Build image ${SERVICE_IMAGE_ID}"
docker build --no-cache -t $SERVICE_IMAGE_ID -f Dockerfile .
echo "Creating container ${SERVICE_CONTAINER_ID}"
docker run --name $SERVICE_CONTAINER_ID --rm -it -e BOOTNODE_ADDRESS="$BOOTNODE_ADDRESS" -p 33445:33445 -p 8545:8545
-p 30303:30303 $SERVICE_IMAGE_ID